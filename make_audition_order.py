...
def rename_long_roles(role_list):
    for key in rename_roles:
        if key in role_list:
            role_list = list(
                map(lambda x: x.replace(key, rename_roles[key]), role_list)
            )
...
# get pair order
pair_order = list(
    pd.DataFrame.from_dict(role_tuples, orient="index")
...
for i in data.index:
    ...
    if len(roles) > 1:
        to_add = pd.DataFrame([data.loc[i, :] * (len(roles) - 1)]).reset_index(
            drop=True
        )
...
def get_row_distance(audition_order_df):
    ...
    for name in names:
        if len(audition_order_df[audition_order_df.name == name].index) > 1:
            row_distance = (
                audition_order_df[audition_order_df.name == name].index[1]
                - audition_order_df[audition_order_df.name == name].index[0]
            )
        else:
            row_distance = 0
...
for key in running_order:
    ...
    for role in running_order[key]:
        if not started:
            ...
            if running_order[key][-1] == role:
                audition_lists[key] = pd.concat(
                    (
                        audition_lists[key],
                        sorted_high_to_low[sorted_high_to_low.roles == role],
                    ),
                    axis=0,
                )
            else:
                ...
                to_add = new_data.loc[
                    ((new_data.roles == role) & (new_data.role_nb == 1)), :
                ]

                # then deal with auditionees with 2 or more auditions
                double_auditionees = new_data.loc[
                    ((new_data.roles == role) & (new_data.role_nb > 1))
                ]
                for auditionee in double_auditionees.name:
                    # if double auditionee already in list, add to top of audition group
                    if (
                        len(audition_lists[key][audition_lists[key].name == auditionee])
                        > 0
                    ):
                        to_add = pd.concat(
                            (
                                double_auditionees[
                                    double_auditionees.name == auditionee
                                ],
                                to_add,
                            )
                        )
    ...
    if len(audition_lists[key]) > 2 & len(audition_lists[key].role.unique()) > 1:
        ...
        while distance_sum_diff > 0:
            first_role_length = len(
                audition_lists[key].loc[
                    audition_lists[key].role == audition_lists[key].loc[0, "role"], :
                ]
            )
            first_new_df = pd.DataFrame(
                np.roll(audition_lists[key], shift=-first_role_length, axis=0),
                columns=audition_lists[key].columns,
            )
            first_distance_sum = get_distance_sum(first_new_df)

            last_role_length = len(
                audition_lists[key].loc[
                    audition_lists[key].role
                    == audition_lists[key].loc[len(audition_lists[key]) - 1, "role"],
                    :,
                ]
            )
            last_new_df = pd.DataFrame(
                np.roll(audition_lists[key], shift=last_role_length, axis=0),
                columns=audition_lists[key].columns,
            )
        ...
        for i in audition_lists[key].index[:-1]:
            if (
                audition_lists[key].loc[i, "name"]
                == audition_lists[key].loc[i + 1, "name"]
            ):
...
for key in audition_lists:
    output_lists[key] = audition_lists[key].merge(
        input_data[
            [
                "name",
                "all roles",
                "Supporting role preference",
                "accept non-auditioned role",
                "ensemble",
            ]
        ],
        on="name",
...
with pd.ExcelWriter(outfile, engine="xlsxwriter") as writer:
    for key in output_lists:
        ...
        if len(output_lists[key]) > 1:
            output_lists[key].sort_index(ascending=False).to_excel(
                writer, sheet_name=f"group_{key}_inv", index=False
            )
